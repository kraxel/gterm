...
static void gterm_new(gterm *gt)
{
    ...
    VteRegex *url_regex;
    GError *err = NULL;
    url_regex = vte_regex_new_for_match("((https?|ftps?|file|news|gopher)://|mailto:)[^\\s'\"<>]+", -1, VTE_REGEX_FLAGS_DEFAULT, &err);
    if (url_regex) {
        ...
        g_warning("Could not create URL regex: %s", err->message);
        g_error_free(err);
    }
